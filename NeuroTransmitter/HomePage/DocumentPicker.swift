 ...
 struct DocumentPicker: UIViewControllerRepresentable {
     ...
     class DocumentPickerCoordinator: NSObject, UIDocumentPickerDelegate {
         ...
         func documentPicker(_ controller: UIDocumentPickerViewController, didPickDocumentsAt urls: [URL]) {
             ...
             // Update the folder's hasDocuments field to true
             if let folder = selectedFolder {
                 updateFolderHasDocuments(folderID: folder.id)
             }

         }
         
         func updateFolderHasDocuments(folderID: UUID) {
             let db = Firestore.firestore()
             let folderReference = db.collection("Folders").document(folderID.uuidString)

             // Update the hasDocuments field to true
             folderReference.updateData(["hasDocuments": true]) { error in
                 if let error = error {
                     print("Error updating hasDocuments field: \(error.localizedDescription)")
                 } else {
                     print("hasDocuments field updated successfully")
                 }
             }
         }
     }
